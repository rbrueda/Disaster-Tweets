...
from tensorflow.keras.layers import Embedding, Conv1D, GlobalMaxPooling1D, Dense, Dropout, MultiHeadAttention, Reshape, Lambda, Input, LayerNormalization, GlobalAveragePooling1D
...
print(df['target'].value_counts())  # for counting occurrences of each class

# Drop missing values in 'text' column
df = df.dropna(subset=['text'])

#preprocessing function for text used
def clean_text(text):
...
#hyperparameters MAX_FEATURES and EMBEDDING_DIM
...
# Get top words from vocabulary in dataframe
...
#build embedding matrix using the max features and embedding dim assigned
embedding_matrix = np.zeros((MAX_FEATURES, EMBEDDING_DIM))
for word, i in word_index.items():
    if i < MAX_FEATURES:
        embedding_matrix[i] = ft_model.get_word_vector(word)  #get our fastetxt vector we downloaded

#data formating - convert text tokens into sequences of indices
df['sequences'] = df['tokens'].apply(lambda tokens: [word_index.get(word, 1) for word in tokens])  # Use 1 for unknown words

#apply Adaptive Padding using pad_sequences built-in function
#X = input
...
#add residual connection for better connections between layers
